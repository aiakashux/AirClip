package com.airclip.airclip.auth
...
private val Context.tokenDataStore: DataStore<Preferences> by preferencesDataStore(name = "airclip_tokens")
 ...
 *   - accountToken: used for REST endpoints (auth, device management)
 *   - deviceToken:  issued by server; kept for future use
 */
object TokenStore {
     ...
     *
     * @return the raw approval-state string or null if absent.
     */
    ...
    /** Persist the device approval state string (e.g. "Approved", "PendingApproval"). */
    ...
    /** Clear device credentials from DataStore and in-memory. Account token is not affected. */
