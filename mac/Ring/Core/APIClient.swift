...
struct ClipHistoryItemRaw: Codable {
...
final class APIClient {
    ...
    func registerDevice(name: String, publicKey: String) async throws -> DeviceRegisterResponse {
        ...
        return try await post(
            ...
            body: ["device_name": name, "public_key": publicKey, "platform": "mac"],
            token: token
        )
    }
    ...
    func fetchMissedClips(afterSeq: Int) async throws -> [ClipHistoryItemRaw] {
        guard let token = AuthManager.shared.deviceToken else { throw APIError.noToken }
        return try await get(path: "/clips?after_seq=\(afterSeq)", token: token)
    }
